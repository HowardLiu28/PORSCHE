...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.block_wise_pool = nn.AvgPool2d(kernel_size=3, stride=7, padding=0)
    ...
    def split_block(self, x, div_parts):
        if div_parts == 0:
            return x
        n, c, w, h = x.size()
        block_size_w = w // div_parts
        block_size_h = h // div_parts
        l = []
        for i in range(div_parts):
            for j in range(div_parts):
                l.append(x[:, :, i * block_size_w: (i + 1) * block_size_w, j * block_size_h: (j + 1) * block_size_h])
        x = torch.cat(l, 0)
        return x
    ...
    def forward(self, x, block=[0, 0, 0, 0], mode=0):

        # x = self.conv1(x)
        # x = self.bn1(x)
        # x = self.relu(x)
        # x1 = self.maxpool1(x)   # x1: [bs*2, 64, 32, 64]
        x1 = x

        if self.training: 
            x = self.split_block(x1, block[0])
            x = self.layer1(x)
            x2 = self.concat_block(x, block[0]) # x2: [bs*2, 256, 32, 64]

            x = self.split_block(x2, block[1])
            x = self.layer2(x)
            x3 = self.concat_block(x, block[1]) # x3: [bs*2, 512, 16, 32]

            x = self.split_block(x3, block[2])
            x = self.layer3(x)
            x4 = self.concat_block(x, block[2]) # x4: [bs*2, 1024, 8, 16]

            x = self.split_block(x4, block[3])
            x = self.layer4(x)
            x5 = self.concat_block(x, block[3]) # x5: [bs*2, 2048, 4, 8]
        else:
            x2 = self.layer1(x1)
            x3 = self.layer2(x2)
            x4 = self.layer3(x3)
