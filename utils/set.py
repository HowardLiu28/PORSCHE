...
import math
import errno
__all__ = ['setup_run', 'set_random_seed', 'accuracy', 'AverageMeter', 'AdamW', 'mixup_data', 'mixup_multidatas', 'total_correlation',
...
def mixup_data(x, y, alpha=1.0, use_cuda=True):
    ...
    if alpha > 0:
        lam = np.random.beta(alpha, alpha)  # 从beta分布中随机采样一个数作为混合系数lam
    else:
        lam = 1

    batch_size = x.size()[0]
    index = torch.randperm(batch_size)  # 生成一个0到batch_size-1的随机排列
    mixed_x = lam * x + (1 - lam) * x[index, :] # 使用lam和1-lam作为权重，对原始输入数据x和经过随机排列的输入数据x[index, :]进行线性插值
    y_a, y_b = y, y[index]
    return mixed_x, y_a, y_b, lam, index


def mixup_multidatas(x1, x2, y, alpha=1.0, use_cuda=True):
    '''Returns mixed inputs, pairs of targets, and lambda'''
    if alpha > 0:
        lam = np.random.beta(alpha, alpha)  # 从beta分布中随机采样一个数作为混合系数lam
    else:
    ...
    index = torch.randperm(batch_size)  # 生成一个0到batch_size-1的随机排列
    mixed_x1 = lam * x1 + (1 - lam) * x1[index, :] # 使用lam和1-lam作为权重，对原始输入数据x和经过随机排列的输入数据x[index, :]进行线性插值
