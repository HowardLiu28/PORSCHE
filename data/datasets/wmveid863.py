    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        for vid in os.listdir(dir_path):
            ...
            for img in r_data:
                ...
                pid = int(vid)
                camid = int(img[1])
                sceneid = int(img[7:10])
                assert 0 <= camid <= 7
                if relabel:
                    pid = vid2label[pid]
                # dataset.append(((r_img_path, n_img_path, t_img_path), pid, camid, sceneid))
                dataset.append(((r_img_path, n_img_path, t_img_path), pid, camid))
        return dataset